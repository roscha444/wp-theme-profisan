...
# ── 5. Setup-Script generieren ──
echo "→ Setup-Script generieren..."
SETUP_TOKEN=$(openssl rand -hex 32)

# Pages JSON als base64 einbetten (sicher für PHP)
...
\$results[] = count( \$options ) . ' Optionen importiert';
...
RESULT=$(curl -sf "$DOMAIN/srk-setup.php?token=$SETUP_TOKEN" 2>&1) || {
